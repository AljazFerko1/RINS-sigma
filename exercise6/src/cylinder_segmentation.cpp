  ...
  if (cloud_cylinder->points.empty ()) 
    ...
    if (h > 0.44 && h < 0.48 && (w > 0.30 && w < 0.34 || w > 0.25 && w < 0.28 || w > 0.15 && w < 0.18)) {
      ...
      if (check) {
        ...
        msg.x = point_map.point.x;
        msg.y = point_map.point.y;
        msg.z = point_map.point.z;
        msg.ori_z = pose.pose.pose.orientation.z;
        msg.ori_w = pose.pose.pose.orientation.w;
        msg.name = "";
...
void pose_cb(geometry_msgs::PoseWithCovarianceStamped data) {
  pose = data;
}
...
main (int argc, char** argv)
{
  ...
  ros::Subscriber pose = nh.subscribe ("/amcl_pose", 1, pose_cb);
  ...
  chatter_pub = nh.advertise<exercise6::Message>("cylinders", 1);
  //cylinder_orientation = nh.advertise<exercise6::Message>("chatter", 1);
