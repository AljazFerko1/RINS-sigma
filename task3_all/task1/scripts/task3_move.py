...
class SpeechTranscriber:
    ...
    def have_a_talk(self, food):
        ...
        if "card" in resp:
            ...
            print(f"I am not waving, because you did not decide to pay with a card or cash.")
        ...
        try:
            ...
            for idx, number in enumerate(["one", "two", "three", "four", "five"]):
                if number in resp:
                    ...
                    break
            if not got_ratting:
                print(f"Y: {self.pretty_str(resp)}, but I did not understand your ratting.")
...
def move_robot():
    ...
    """goal.target_pose.header.frame_id = "map"
    goal.target_pose.pose.orientation.z = -1
    goal.target_pose.pose.position.x = 0.98
    ...
    while min < max:
        ...
        if object == "cylinder": 
            ...
            while True:
                ...
                (_, _, theta) = euler_from_quaternion([last_feedback.pose.pose.orientation.x, last_feedback.pose.pose.orientation.y, last_feedback.pose.pose.orientation.z, last_feedback.pose.pose.orientation.w])
...
def face_position(msg):
    ...
    faces_dict[msg.name] = ((last_feedback.pose.pose.position.x + v1[0] * vec1, last_feedback.pose.pose.position.y + v1[1] * vec1), (last_feedback.pose.pose.orientation.z, last_feedback.pose.pose.orientation.w))
...
if __name__ == '__main__':
    ...
    cmd_vel = rospy.Publisher('/cmd_vel_mux/input/navi', gmsg.Twist, queue_size=1)
